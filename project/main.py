...
# test
import json
FAKE_DATA_PATH = os.path.join(os.path.dirname(__file__), 'fake_data.json')
def load_fake_data():
    # JSON 데이터 불러오기 함수
    with open(FAKE_DATA_PATH, 'r', encoding='utf-8') as file:
...
async def asyncgenProject(userinfo: UserInfo):
    ...
    summarized_job_posting = fake_data["summarized_job_posting"]
    themes = fake_data["themes"]
    res = fake_data["res"]
